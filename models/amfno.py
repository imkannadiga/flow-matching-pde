...
class AMFNOBlock(nn.Module):
    """Spectral conv block with FiLM conditioning."""
...
class AMFNO(PDEModel):
    """
    Amortized Multi-scale FNO.

    Conditioning path : cond [B, C, H, W] → global avg-pool → MLP → context
                        vector → FiLM scale/shift applied after every spectral block.
    ...
    def __init__(
        ...
        out_channels=None,
        n_layers=4,
        **kwargs,  # absorbs stray Hydra keys (e.g. name, separable)
    ):
        super().__init__()
        del kwargs
        ...
        cond_dim = 1 + self.context_dim  # t scalar + amortized context

        self.p = nn.Conv2d(in_channels, hidden_channels, 1)
        self.blocks = nn.ModuleList([
            AMFNOBlock(hidden_channels, modes_[0], modes_[1], cond_dim)
    ...
    def forward(self, u, cond, t):
        ...
        if t.dim() == 0 or t.numel() == 1:
            t = t.expand(B)
        cond_vector = torch.cat([t.unsqueeze(1).float(), ctx], dim=1)  # [B, 1+context_dim]
